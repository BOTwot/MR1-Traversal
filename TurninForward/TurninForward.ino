...
int noadj = 0;
int32_t pwmfw, pwmbk, pwmlt, pwmrt, pwm, pwmthr, pwmcl, pwmccl;    //32 bit int used to avoid the overflow in throttle mode
...
{
  straight.run();
  if ((stcorr1 + pwmrt - stcorr2 - pwmlt + pwmcl - pwmccl) > 0)
  {
    analogWrite(NOSE_PIN1, stcorr1 + pwmrt - stcorr2 - pwmlt + pwmcl - pwmccl);
    analogWrite(NOSE_PIN2, 0);
    //Serial.print("if ");
    //Serial.println(stcorr1 + pwmrt + sdrcorr1 - stcorr2 - pwmlt - sdlcorr2);

  }
  else if ((stcorr1 + pwmrt - stcorr2 - pwmlt + pwmcl - pwmccl) < 0)
  {
    analogWrite(NOSE_PIN1, 0);
    analogWrite(NOSE_PIN2, pwmccl + stcorr2 + pwmlt - pwmcl - stcorr1 - pwmrt );
    //Serial.print("in else ");
    //Serial.println(stcorr2 + pwmlt + sdlcorr2 - stcorr1 - pwmrt - sdrcorr1);
    //Serial.print("out else ");
  }
  if ((pwmcl + pwmfw + stcorr1 + 0.5 * pwmlt  - pwmccl - 0.5 * pwmrt - pwmbk - stcorr2) > 0)
  {
    analogWrite(LEFT_PIN1, pwmcl + pwmfw + stcorr1 + 0.5 * pwmlt  - pwmccl - 0.5 * pwmrt - pwmbk - stcorr2);
    analogWrite(LEFT_PIN2, 0);
    //Serial.print("if ");
    //Serial.println(pwmfw + 0.5 * pwmlt + sdlcorr1 - 0.5 * pwmrt - pwmbk);
  }
  else if ((pwmcl + pwmfw + stcorr1 + 0.5 * pwmlt  - pwmccl - 0.5 * pwmrt - pwmbk - stcorr2) < 0)
  {
    analogWrite(LEFT_PIN1, 0);
    analogWrite(LEFT_PIN2,  pwmccl + 0.5 * pwmrt + pwmbk + stcorr2 - pwmcl - pwmfw - stcorr1 - 0.5 * pwmlt );
    //Serial.print("in else left ");
    //Serial.println(0.5 * pwmrt + pwmbk - pwmfw - 0.5 * pwmlt - sdlcorr1);
  }
  if ((pwmcl + pwmfw + stcorr2 + 0.5 * pwmrt - pwmccl - stcorr1 - 0.5 * pwmlt  - pwmbk) > 0)
  {
    analogWrite(RIGHT_PIN1, pwmcl + pwmfw + stcorr2 + 0.5 * pwmrt - pwmccl - stcorr1 - 0.5 * pwmlt  - pwmbk);
    analogWrite(RIGHT_PIN2,  0);
    //Serial.print("if right ");
    //Serial.println(pwmfw - 0.5 * pwmlt + 0.5 * pwmrt + sdrcorr2 - pwmbk);
  }
  else if ((pwmcl + pwmfw + stcorr2 + 0.5 * pwmrt - pwmccl - stcorr1 - 0.5 * pwmlt  - pwmbk) < 0)
  {
    analogWrite(RIGHT_PIN1, 0);
    analogWrite(RIGHT_PIN2, pwmccl + stcorr1 + 0.5 * pwmlt  + pwmbk - pwmcl - pwmfw - stcorr2 - 0.5 * pwmrt );
...
void loop() {
  ...
  else if (IBus.readChannel(7) == 2000) // Throttle Mode
  {
  ...
  else if (IBus.readChannel(5) == 2000) //Turn in forward mode
  {
    IBus.loop();    //Call always in loop to recieve signal from remote
    //Condition for stop below
    noadj = 0;
    ...
    while (IBus.readChannel(1) > 1535)     //forward
    {
      ...
      if (IBus.readChannel(3) > 1535)     //rotation clockwise
      {
        pwmcl = abs(IBus.readChannel(3) - 1500 );
        pwmcl = pwmcl * pwmthr / 500;
        noadj = 1;
      }
      if (IBus.readChannel(3) > 1535)     //rotation anti- clockwise
      {
        pwmccl = abs(1500 - IBus.readChannel(3));
        pwmccl = pwmccl * pwmthr / 500;
        noadj = 1;
      }
      if (IBus.readChannel(0) > 1535) //right
      {
