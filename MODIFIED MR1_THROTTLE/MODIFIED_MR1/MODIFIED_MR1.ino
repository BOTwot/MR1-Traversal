...
#define NOSE_PIN1 OCR3C //3
#define NOSE_PIN2 OCR3A //5
#define LEFT_PIN1 OCR4A //6
#define LEFT_PIN2 OCR4B //7
#define RIGHT_PIN1 OCR4C//8
#define RIGHT_PIN2 OCR1A //11
...
{
  pwm = map(pwm, 0, 500, 0, 296);   //to decrease the speed of yaw
  pwmfw = map(pwmfw, 0, 500, 0, 296);
  pwmbk = map(pwmbk, 0, 500, 0, 296);
  pwmlt = map(pwmlt, 0, 500, 0, 296);
  pwmrt = map(pwmrt, 0, 500, 0, 296);

}
void mapforthr()    //mapping for throttle, only till 1600 to avoid unnecessary movement of stick
{
  pwmthr = map(pwmthr, 1000, 1600, 0, 380);
...
void forward()      //Call the function for forward movement, adjust is included
{
  ...
  {
    NOSE_PIN1 = (stcorr1 + pwmrt + sdrcorr1 - stcorr2 - pwmlt - sdlcorr2);
    NOSE_PIN2 = 0;
    //Serial.print("if ");
  ...
  {
    NOSE_PIN1 = 0;
    NOSE_PIN2 = ( stcorr2 + pwmlt + sdlcorr2 - stcorr1 - pwmrt - sdrcorr1 );
  ...
  {
    LEFT_PIN1 = ( pwmfw + 0.5 * pwmlt + sdlcorr1 - 0.5 * pwmrt - pwmbk);
    LEFT_PIN2 = 0;
    //Serial.print("if ");
  ...
  {
    LEFT_PIN1 = 0;
    LEFT_PIN2 = ( 0.5 * pwmrt + pwmbk - pwmfw - 0.5 * pwmlt - sdlcorr1 );
  ...
  {
    RIGHT_PIN1 = (pwmfw - 0.5 * pwmlt + 0.5 * pwmrt + sdrcorr2 - pwmbk);
    RIGHT_PIN2 = 0;
  ...
  {
    RIGHT_PIN1 = 0;
    RIGHT_PIN2 = ( 0.5 * pwmlt + pwmbk - 0.5 * pwmrt - sdrcorr2 - pwmfw);
